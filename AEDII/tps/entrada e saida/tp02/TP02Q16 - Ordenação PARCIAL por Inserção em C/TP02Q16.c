...
int compare(Jogador a, Jogador b)
{
	...
	{
		//  printf("%s %s - %s %s / %d\n", a.anoNascimento, a.nome, b.anoNascimento, b.nome, strcmp(a.nome, b.nome));
...
void insercao(Jogador jogadores[], int n)
{
	for (int i = 1; i < n; i++)
	{
		...
		while ((j >= 0) && compare(jogadores[j], tmp) > 0)
		{
...
int main()
{
	...
	//abrindo o arquivo
	pont_arq = fopen("633516_radixsort.txt", "w");
