...
void quicksortRec(Jogador jogadores[], int esq, int dir)
{
	...
	while (i <= j)
	{
		while (compare(jogadores[i], pivo) <= -1)
			i++;
		while (compare(jogadores[j], pivo) >= 1)
			j--;
		if (i <= j)
		{
			swap(&i, &j, jogadores);
	...
	if (i < dir)
		quicksortRec(jogadores, i, dir);
}

void quicksort(Jogador jogadores[], int n)
{
...
int main()
{
	...
	//abrindo o arquivo
	pont_arq = fopen("633516_quicksort.txt", "w");
